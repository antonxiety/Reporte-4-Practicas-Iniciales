...
function Perfil() {
  ...
  async function cargarPerfil() {
    ...
    setError('');
    try {
      const { data } = await api.get(`/users/${carne}`);
      setPerfil(data);
    } catch (err) {
  ...
  const cursosAprobados = perfil.cursosAprobados || [];
  const totalCreditos = perfil.totalCreditos ?? cursosAprobados.reduce((sum, c) => sum + (c.creditos || 0), 0);
